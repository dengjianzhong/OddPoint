...
public class JavaWebSocket implements IWebSocket {
    ...
    static {
        ...
        handlerThread.start();
        wordHandler = new Handler(handlerThread.getLooper());
    }

    private static Handler wordHandler;
    ...
    public void joinRoom(String room) {
        ...
        wordHandler.post(new Runnable() {
            @Override
            public void run() {
                try {
                    while (!mWebSocketClient.getReadyState().equals(mWebSocketClient.getReadyState().OPEN)) {
                        Log.d("==>", "turn连接中···");
                        Thread.sleep(500);
                    }
